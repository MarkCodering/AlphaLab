...
export const ToolInvocationSchema = z.object({
  ...
  approvalDigest: z.string().regex(/^sha256:[a-f0-9]{64}$/).optional(),
...
export function authorizeToolInvocation(
  ...
  if (localOnly && manifest.dataBoundary === 'EXTERNAL') {
    throw new ToolPolicyError('EXTERNAL_TOOL_FORBIDDEN', 'External tools are disabled in local-only mode');
  }
  ...
  if (undeclaredHosts.length > 0) {
    throw new ToolPolicyError('NETWORK_HOST_FORBIDDEN', `Undeclared hosts: ${undeclaredHosts.join(', ')}`);
  }
  if (manifest.riskTier === 'RED' && !invocation.approvalDigest) {
    throw new ToolPolicyError('APPROVAL_REQUIRED', 'Red tool invocations require an approval digest');
  }
}
